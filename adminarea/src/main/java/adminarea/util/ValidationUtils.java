...
public class ValidationUtils {
    private static final Pattern AREA_NAME_PATTERN = Pattern.compile("^[a-zA-Z0-9][a-zA-Z0-9_-]*$");
    ...
    public static void validateAreaName(String name) {
        ...
        if (name.length() < MIN_NAME_LENGTH) {
            throw new IllegalArgumentException("Area name must be at least " + MIN_NAME_LENGTH + " characters");
        }
        
        if (name.length() > MAX_NAME_LENGTH) {
            throw new IllegalArgumentException("Area name cannot exceed " + MAX_NAME_LENGTH + " characters");
        }
        
        // Simplify validation by using one comprehensive check
        if (!name.matches("^[a-zA-Z0-9][a-zA-Z0-9_-]*$")) {
            throw new IllegalArgumentException("Area name must start with a letter or number and can only contain letters, numbers, underscores, and hyphens");
        }
        
        // Check for consecutive special characters
        if (name.contains("__") || name.contains("--") || name.contains("_-") || name.contains("-_")) {
            throw new IllegalArgumentException("Area name cannot contain consecutive special characters (__,--,_-,-_)");
        }
    }
