...
/**
 * Custom form for editing items and drops permissions of an area.
 */
public class ItemsDropsHandler {
    ...
    /**
     * Open the items and drops settings form for a player with no pre-selected area
     */
    ...
    public void open(Player player, Area area) {
        ...
        try {
            ...
            try {
                ...
                if (freshArea != null) {
                    ...
                    if (plugin.isDebugMode()) {
                        plugin.debug("Reloaded area from database for items/drops settings form");
                    }
                }
            } catch (Exception e) {
            ...
            form.addHandler(FormResponseHandler.withoutPlayer(ignored -> {
                ...
                try {
                    ...
                    if (changedCount > 0) {
                        ...
                        FormUtils.updateAreaAndNotifyPlayer(player, finalArea, changedCount, FormIds.EDIT_AREA, "items/drops");
                    } else {
                    ...
                    plugin.getLogger().error("Error processing items/drops settings form", e);
            ...
            plugin.getFormIdMap().put(player.getName(), new adminarea.data.FormTrackingData(
                FormIds.ITEMS_DROPS_SETTINGS, System.currentTimeMillis()));
            
        } catch (Exception e) {
            plugin.getLogger().error("Error creating items/drops settings form", e);
            player.sendMessage(plugin.getLanguageManager().get("messages.form.error.generic"));
            FormUtils.cleanup(player);
        }
    }
